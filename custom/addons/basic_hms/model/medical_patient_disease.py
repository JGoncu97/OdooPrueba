...
class medical_patient_disease(models.Model):
    ...
    _description = 'Enfermedad del paciente'
    ...
    disease_severity = fields.Selection([('1_mi','Leve'),
                               ('2_mo','Moderada'),
                               ('3_sv','Severa')], string='Severidad')
    status = fields.Selection([('c','Crónica'),
                               ('s','Estado actual'),
                               ('h','Curada'),
                               ('i','Mejorando'),
                               ('w','Empeorando')], string='Estado de la enfermedad')
    ...
    doctor_id = fields.Many2one('medical.physician', string='Médico')
    is_allergic = fields.Boolean(string='Enfermedad alérgica')
    allergy_type = fields.Selection([('da','Alergia a medicamentos'),
                               ('fa','Alergia alimentaria'),
                               ('ma','Alergia miscelánea'),
                               ('mc','Contraindicación miscelánea')], string='Tipo de alergia')
    pregnancy_warning = fields.Boolean(string='Advertencia durante embarazo')
    week_of_pregnancy = fields.Integer(string='Contraída en la semana de embarazo #')
